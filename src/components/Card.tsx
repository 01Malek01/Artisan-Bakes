...
export default function Card({image,title,description,direction,button,buttonBgColor,buttonColor,buttonStyles,imageOrder}:Props) {
  return (
    <div className={`flex gap-5 items-center lg:items-start justify-between max-width-[300px] min-width-[150px] max-height-[300px] min-height-[150px] mt-5 ${direction ? direction === 'row' ? 'flex-row' : 'flex-col' : 'flex-col'}`}> 
      ...
      <p className='text-sm max-w-[300px] opacity-50 text-balance lg:text-left text-center text-[#8A7561]'>{description}</p>
